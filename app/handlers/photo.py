...
async def _process_and_reply(file_paths: list[str], status: Message, user_id: int, username: str):
    ...
    await status.edit_text(t("analyzing_photo", lang, label=label))
    data = await analyzer.analyze(file_paths)

    if "error" in data:
        await status.edit_text(t("analysis_error", lang, error=data["error"]))
        return
    ...
    await status.edit_text(format_reply(data, receipt.id, n, lang), parse_mode="HTML")
...
async def _process_album(group_id: str, status: Message, bot: Bot, user_id: int, username: str):
    try:
        ...
        await _process_and_reply(file_paths, status, user_id, username)
    finally:
...
async def handle_photo(message: Message, bot: Bot):
    ...
    if group_id:
        ...
        status = await message.answer(t("processing_receipt", lang))
        await _process_and_reply([file_path], status, message.from_user.id, message.from_user.username)
